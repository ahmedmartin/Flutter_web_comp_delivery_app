...
class Delivery_history extends StatefulWidget{
  ...
  State<StatefulWidget> createState() {
    return _Delivery_history();
  }


}

class _Delivery_history extends State<Delivery_history>{
  ...
  get_dates() async {
    ...
    coll_ref = FirebaseFirestore.instance.collection('companies').doc(comp_id).collection('delivery_money');
                  ...
                  setState(() {
                    ...
                    if(search_controller.text.isNotEmpty)
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text("اختار اسم المندوب",style: TextStyle(fontSize: 40),textAlign: TextAlign.center,),
  ...
  get_data(){
    coll_ref.doc(selected_date).collection('all').orderBy('name').startAt([search_controller.text.toUpperCase()]).endAt([search_controller.text.toUpperCase()+ '\uf8ff'])
        .startAt([search_controller.text]).endAt([search_controller.text+ '\uf8ff']).get().then((snap_shot){
